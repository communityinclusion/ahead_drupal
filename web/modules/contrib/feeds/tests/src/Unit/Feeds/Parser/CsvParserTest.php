...
class CsvParserTest extends FeedsUnitTestCase {
  ...
  /**
   * Tests parsing a file with rows that contain the csv delimiter only.
   */
  public function testFeedWithBlankColumns() {
    $this->feedType->method('getMappingSources')
      ->willReturn([]);
    ...
    $file = $this->resourcesPath() . '/csv/with-empty-columns.csv';
    $fetcher_result = new FetcherResult($file);
